...
- `Overview`: quick metrics for universe size, holdings count, output coverage, missing-data warnings, local fundamentals coverage, DCF-ready count, peer-ready count, research-health readiness, a holdings-first unlock board for blocked portfolio names, a theme-first unlock board for grouped research priorities, and a compact market-context strip from local benchmark-relative theme output
